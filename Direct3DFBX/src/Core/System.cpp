...
void System::UpdateUpdatables(float dt, bool current_flag)
{
	...
	for (auto & i : mUpdatableRunList)
	{
		...
		AddJobs(i->GetUpdateOrder(),
			Delegate< void(IUpdatable *, float, bool) >::FromFunction< &System::UpdateDelegate >(),
			i, dt, current_flag);
	}
}
...
void System::UpdateDelegate(IUpdatable * updatable, float dt, bool current_flag)
{
	updatable->Update(dt);
...
void System::DrawDrawables(float dt)
{
	// Remove invisible
	RemoveInVisible();
