...
bool VerticalBlurShader::Initialize(ID3D11Device* device, HWND hwnd, const D3D_SHADER_MACRO* macros)
{
	bool result;
	// Initialize the vertex and pixel shaders.
	std::string csshaderName = "Shader/VerticalBlurShader.hlsl";
...
bool VerticalBlurShader::Render(ID3D11DeviceContext* deviceContext, int indexCount)
{
	bool result;

	// Set the shader parameters that it will use for rendering.
	FAIL_CHECK_WITH_MSG(result, "VerticalBlurShader Render Fail");
...
bool VerticalBlurShader::InitializeShader(ID3D11Device* device, HWND hwnd, string csFilename, const D3D_SHADER_MACRO* macros)
{
	HRESULT hr = S_OK;
	...
	CompileShaderFromFile(COMPUTE_SHADER, csFilename.c_str(), "VerticalBlurShader", "cs_5_0", &device, &computeShaderBuffer, macros, &m_computeShader);

	FAIL_CHECK_BOOLEAN_WITH_MSG(hr, "VerticalBlurShader input layout creation failed");
	...
	FAIL_CHECK_BOOLEAN_WITH_MSG(hr, "VerticalBlurShader resolution buffer creation failed");
...
bool VerticalBlurShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, XMINT3 resolution)
{
	HRESULT result;
	...
	// weight
	{
		...
		// Copy the matrices into the constant buffer.
		memcpy(dataPtr->weights, mWeights, 2 * MAX_KERNEL_RADIUS + 1);
...
void VerticalBlurShader::SetBlurKernelRadius(int radius)
{
...
void VerticalBlurShader::RenderShader(ID3D11DeviceContext* deviceContext)
{
