...
DepthRenderTarget::DepthRenderTarget(const DepthRenderTarget& other)
{
}
...
bool DepthRenderTarget::Initialize(ID3D11Device* device, int width, int height)
{
...
// Shutdown releases the three interfaces used by the MultiDepthRenderTargetClass.
...
// The SetDepthRenderTarget function sets the render target view in this class as the current rendering location for all graphics to be rendered to.
void DepthRenderTarget::SetRenderTarget()
{
	ID3D11DeviceContext* deviceContext = GRAPHICS.GetPipeline()->GetContext();
	ID3D11DepthStencilView* depthStencilView = GRAPHICS.GetPipeline()->GetDepthStencilView();
...
void DepthRenderTarget::ReleaseRenderTarget(eShaderType shaderType)
{
	ID3D11DeviceContext* deviceContext = GRAPHICS.GetPipeline()->GetContext();
...
void DepthRenderTarget::ClearRenderTarget( const float* color)
{
	ID3D11DeviceContext* deviceContext = GRAPHICS.GetPipeline()->GetContext();
	ID3D11DepthStencilView* depthStencilView = GRAPHICS.GetPipeline()->GetDepthStencilView();
...
void DepthRenderTarget::ClearRenderTarget(float r, float g, float b, float a)
{
	ID3D11DeviceContext* deviceContext = GRAPHICS.GetPipeline()->GetContext();
	ID3D11DepthStencilView* depthStencilView = GRAPHICS.GetPipeline()->GetDepthStencilView();
