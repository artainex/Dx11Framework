...
bool MultiRenderTarget::Initialize(int textureWidth, int textureHeight)
{
...
void MultiRenderTarget::ReleaseRenderTargets(eShaderType shadertype)
{
	ID3D11DeviceContext* deviceContext = GRAPHICS.GetPipeline()->GetContext();
...
void MultiRenderTarget::ClearRenderTargets()
{
	...
	for(auto &iter : m_renderTargetViews)
		deviceContext->ClearRenderTargetView(iter, ClearColor);
