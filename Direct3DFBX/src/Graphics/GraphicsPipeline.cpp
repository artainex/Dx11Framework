...
/// @brief		Graphics pipeline for FBX Loader tester application
/// 
...
bool GraphicsPipeline::Initialize(UINT screen_width,
	UINT screen_height,
	HWND window,
	float screen_near,
	float screen_far,
	...
	bool vsync)
{
	...
	{
		if (displayModeList[i].Width == static_cast<UINT>(screen_width))
		{
			if (displayModeList[i].Height == static_cast<UINT>(screen_height))
			{
	...
	mFOV = static_cast<float>(PI) / 4.f;
	mAspectRatio = static_cast<float>(screen_width) / static_cast<float>(screen_height);

	// Create the projection matrix for 3D rendering.
	mProjectionMatrix = CreatePerspectiveFieldOfView(mFOV, mAspectRatio, screen_near, screen_far);

	// Create an orthographic projection matrix for 2D rendering.
	mOrthoMatrix = CreateOrthographic(static_cast<float>(screen_width), static_cast<float>(screen_height), screen_near, screen_far);
...
void GraphicsPipeline::BeginScene(const XMFLOAT4 & color)
{
	float fcolors[4]{ color.x, color.y, color.z, color.w };
...
XMMATRIX GraphicsPipeline::GetWorld() const
{
	return mWorldMatrix;
}

XMMATRIX GraphicsPipeline::GetProjection() const
{
	return mProjectionMatrix;
}

XMMATRIX GraphicsPipeline::GetOrthogonal() const
{
	return mOrthoMatrix;
}

void GraphicsPipeline::SetWorld(const XMMATRIX & world)
{
	mWorldMatrix = world;
}

void GraphicsPipeline::SetProjection(const XMMATRIX & projection)
{
	mProjectionMatrix = projection;
}

void GraphicsPipeline::SetOrthogonal(const XMMATRIX & orthogonal)
{
...
void GraphicsPipeline::DrawInForeground(void)
{
	// Setup the viewport for rendering.
	mViewport.MinDepth = 0.0f;
...
void GraphicsPipeline::DrawInBackground(void)
{
	// Setup the viewport for rendering.
...
//Change Depth value
void GraphicsPipeline::SetDepthRange(const float& MinZ, const float& MaxZ)
{
	// Setup the viewport for rendering.
...
void GraphicsPipeline::SetDefaultDepthRange(void)
{
	// Setup the viewport for rendering.
	mViewport.MinDepth = 0.0f;
	mViewport.MaxDepth = 1.0f;
