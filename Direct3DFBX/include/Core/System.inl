...
T * System::AddGameComponent(const char * name)
{
	...
	//BH_STATIC_ASSERT((std::is_base_of<GameComponent, T>::value == true), L"System::AddGameComponent - T is not a derived class of GameComponent!");
...
{
	//// Compile time check for hierachy
	//BH_STATIC_ASSERT((std::is_base_of<GameComponent, T>::value == true), L"System::GetGameComponent - T is not a derived class of GameComponent!");
...
void System::AddJobs(unsigned int priority, const Delegate< ReturnType(Arguments...) > & delegate, Arguments... args)
{
	Message msg;
	...
	// Add to job queue
	mJobQueue.AddJobs(priority, JobQueue::Job::FromFunction< &detail::WorkImpl< Arguments..., Delegate< ReturnType(Arguments...) > > >(), msg);
}
