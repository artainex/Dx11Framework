...
public:
	...
	void AddJobs(unsigned int priority, const Delegate< ReturnType(Arguments...) > & delegate, Arguments... args);

	// Update all active updatables (current flag is something like bit flipping - to know is component updated in this frame)
	void UpdateUpdatables(float dt, bool current_flag);
...
private:
	...
	static void UpdateDelegate(IUpdatable * updatable, float dt, bool current_flag);
...
protected:
	...
	typedef std::unordered_map< CName, UINT > GameComponentProcessMap;
...
protected:
	...
	const char * mSystemName;								//!< For debugging
};
